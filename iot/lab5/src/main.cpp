...
bool isGreenBtnPressed() {
  static int greenStableState = HIGH;
  static int greenPreviousState = HIGH;
  static unsigned long greenDebounceTimer = 0UL;
  bool greenPressed = false;

  int currentGreenButtonState = digitalRead(BUTTON_PIN_GREEN);

  if (greenPreviousState != currentGreenButtonState) {
    greenDebounceTimer = millis();
  }

  if ((millis() - greenDebounceTimer) > DEBOUNCE_INTERVAL) {
    if (currentGreenButtonState != greenStableState) {
      if (greenStableState == HIGH && currentGreenButtonState == LOW) {
        greenPressed = true;
      }
      greenStableState = currentGreenButtonState;
    }
  }

  greenPreviousState = currentGreenButtonState;
